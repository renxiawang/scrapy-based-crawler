...
class ScheduleSpider(CrawlSpider):
    ...
    rules = (
        Rule(SgmlLinkExtractor(allow=(r'http://espn\.go\.com/nba/boxscore\?id=.*',)), callback='parse_result'),
        Rule(SgmlLinkExtractor(allow=(r'.*',), restrict_xpaths=("//div[@class=\"floatleft\"]/a[1]",)), callback='parse_item', follow=True),
        )
    ...
    def parse_item(self, response):
        ...
        for table in tables:
            ...
            try:
                game_date = table.xpath("tr[1]/td/text()").extract()[0].split(',')[1].strip()
                ...
                for row in table_rows:
                    ...
                    game['time'] = row.xpath("td[2]/text()|td[2]/a/text()").extract()[0] + ' ET'
                    yield game
            except Exception, e:
                raise e
    ...
    def parse_result(self, response):
        ...
        try:
            datetime = sel.xpath("//*[@id=\"gamepackageTop\"]/div[4]/div/p[1]/text()").extract()[0] 

            game['time'] = datetime.split(',')[0]
            game['date'] = (','.join(datetime.split(',')[1:])).strip()
            game['away'] = sel.xpath("//*[@id=\"gamepackageTop\"]/div[5]/div[2]/p[2]/span/text()").extract()[0].strip(': ')
            game['home'] = sel.xpath("//*[@id=\"gamepackageTop\"]/div[5]/div[2]/p[3]/span/text()").extract()[0].strip(': ')
